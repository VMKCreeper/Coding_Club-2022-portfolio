...
class Game:
    ...
    def ability_icons(self, surface: pygame.Surface, player: Player, x: int, y: int) -> None:
    ...
    def collision_tiles(self, collisionTile: List[pygame.Rect], platform: List[pygame.Rect], slime: List[pygame.Rect]) -> None:  # Max loops
        for i in range(len(self.map)):
            for j in range(len(self.map[i])):
    ...
    def check_win(self, gamemode: int, p1: Player, p2: Player, game_timer: Timer, highscore: List[int]) -> bool:  # Max return value
    ...
    def draw_ability_list(self, surface: pygame.Surface, p1_coords_list: List[int], p2_coords_list: List[int], p1: Player, p2: Player) -> None:
