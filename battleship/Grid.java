public class Grid
{
    ...
    public String[][] gameGrid = new String[NUM_ROWS][NUM_COLS];
    ...
    public void printShips()
    {
        ...
        for(int row = 0; row < 10; row++)
        {
            ...
            for(int col = 0; col < 10; col++)
            {
                boolean status = grid[row][col].hasShip();
                if(status)
                {
                    gameGrid[row][col] = "X";
                }
    ...
    public void addShip(Ship s)
    {
        ...
        for(int i = 0; i < s.getLength(); i++)
        {
            ...
            grid[s.getRow() + row][s.getCol() + col].setShip(true);
        }
    }
    
    public boolean available(Ship s)
    {
        int row = 0;
        int col = 0;
        boolean yes = true;
        ...
        for(int i = 0; i < s.getLength(); i++)
        {
            ...
            if(grid[s.getRow() + row][s.getCol() + col].hasShip())
            {
                return false;
            }
        }
